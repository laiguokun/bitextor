...
######################################################################################
# helpers
def GetNextState(curr, action, goal, F):
    if action == 0:
        next = curr - 5
    elif action == 1:
        next = curr + 1
        ...
        next = curr
    #assert(next >= 0)
    #print(curr, next, action)

    if next < 0 or next >= 15 or F[curr][next] == 0:
        reward = -100
    elif next == goal:
        reward = 6
    elif action == 4:
...
def get_rnd_next_state(s, F, ns, goal):
    ...
    action = actions[i]
    next_state, reward = GetNextState(s, action, goal, F)
...
def Walk(start, goal, Q, F):
    ...
    while True:
        ...
        next, reward = GetNextState(curr, action, goal, F)
        totReward += reward
...
######################################################################################
def GetMaxQ(Q, actions, next_s, goal, F):
    max_Q = -9999.99

    if next_s < 0 or next_s >= 15:
        return max_Q

    for j in range(len(actions)):
        nn_a = actions[j]
        nn_s, _ = GetNextState(next_s, nn_a, goal, F)

        if nn_s < 0 or nn_s >= 15 or F[next_s][nn_s] == 0:
            q = -100
        else:
            q = Q[next_s, nn_a]
...
def Trajectory(curr_s, F, Q, gamma, lrn_rate, goal, ns):
    while (True):
        ...
        #print(curr_s, next_s, action, reward)
        actions = get_poss_next_actions(next_s, F, ns)

        DEBUG = False
        #DEBUG = curr_s == 3

        max_Q = GetMaxQ(Q, actions, next_s, goal, F)
        ...
        if DEBUG:
            ...
            print("Q", curr_s, next_s, action, reward, before, after)
